...
export const updateFileCheck = (req, res, next) => {
  ...
  if (file) {
    if(validExts.includes(file.mimetype)) {
      if(!req.body.prevImage)  return res.status(400).json({status: 'error', message: `please provide old imagePath`});
    ...
    next();
      ...
      return res.status(400).json({status: 'error', message: `please provide valid image`})
    }
  } else {
next();
}
